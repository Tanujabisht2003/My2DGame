	...
	public void attacking() {
		...
		if(spriteCounter > motion1_duration && spriteCounter <= motion2_duration) {
			...
			gp.player.damageMonster(monsterIndex,this, attack, currentWeapon.knockBackPower);
			 ...
			 int projectileIndex = gp.cChecker.checkEntity(this, gp.iTile);
		...
		if(worldX + gp.tileSize > gp.player.worldX - gp.player.screenX && 
		   ...
		   worldY - gp.tileSize < gp.player.worldY + gp.player.screenY) {
			...
			if(type == 2 && hpBarOn == true) {
				...
				double hbBarValue = oneScale*life;
				g2.setColor(new Color(35,35,35));
				g2.fillRect(screenX - 1, screenY - 16, gp.tileSize+ 2, 12);
				
				g2.setColor(new Color(255,0,30));
				g2.fillRect(screenX, screenY - 15, (int)hbBarValue,10);
	...
	public int getDetected(Entity user, Entity target[][], String targetName) {
		...
		for(int i= 0;i < target[1].length; i++) {
			if(target[gp.currentMap][i] != null) {
				if(target[gp.currentMap][i].getCol() == col && target[gp.currentMap][i].getCol() == row && target[gp.currentMap][i].name.equals(targetName)) {
					index = i;
					break;
				}
			}
